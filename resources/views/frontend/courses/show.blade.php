        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div class="blog_content">
                    <div class="blog_title">{{ $course->title }}</div>
                    <div class="blog_meta">
                        <ul>
                            <li>{{ __('Sana') }} <a href="/"><i class="fa fa-calendar"></i> {!! $new_date = date('d-m-Y', strtotime($course->created_at)) !!}</a></li>
                            <li><a href="/">Admin</a></li>
                        </ul>
                    </div>
                    <div class="blog_image"><img src="{{ asset('storage/' . $course->photo) }}" alt=""></div>
                    <p>{{ $course->content }}</p>                    
                </div>
                <div class="blog_extra d-flex flex-lg-row flex-column align-items-lg-center align-items-start justify-content-start">
                    <div class="blog_tags">
                        <span>{{ __('Sana') }}: </span>
                        <ul>
                            <li><a href="/"><i class="fa fa-calendar"></i> {!! $new_date = date('d-m-Y', strtotime($course->created_at)) !!}</a></li>
                        </ul>
                    </div>
                    <div class="blog_tags">
            ...
            <div class="col-lg-4">
                <div class="sidebar">
                    ...
                    <!-- Latest News -->
                    <div class="sidebar_section">
                        <div class="sidebar_section_title">{{ __('Tillar') }}</div>
                        <div class="sidebar_latest">
                            @foreach ($recent_languages as $language)
                            <div class="latest d-flex flex-row align-items-start justify-content-start">
                                <div class="latest_image"><div><img src="{{ asset('storage/' . $language->photo) }}" alt=""></div></div>
                                <div class="latest_content">
                                    <div class="latest_title"><a href="{{ route('front.languages.show', ['language' => $language->id]) }}">{{ $language->title }}</a></div>
                                    <div class="latest_price"><a href="{{ route('front.languages.show', ['language' => $language->id]) }}">{{ __('Ko\'rish') }}</a></div>
                                </div>
                            </div>
                            @endforeach
                        </div>
